...
const Skills = ({ icons }) => {
	...
	const ref = useD3((svg) => {
		...
		svg
			...
			.attr('class', 'h-16 w-16 rounded-full translate-x-[-50%] translate-y-[-50%] absolute hover:text-primary-light/100 text-primary-light/0 hover:text-opacity-100 z-0 hover:z-50 text-center')
			.html((data) => {return data.svg})
				.append('i')
				.attr('class', 'font-poppins text-sm')
				.attr('id', 'skill-name')
				.html((data) => { return data.title });
